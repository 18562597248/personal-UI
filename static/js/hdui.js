...
export {InitiateSideMenu, InitiateSettings, InitiateWidgets, required_validate, Notify}
...
function InitiateSettings() {
    ...
    /* hd js begin - add by gcm*/
    /* 增加setting中Copyright的cookie设置,如果选中就显示页脚*/
    if (readCookie("page-footer-copyright") != null) {
        if (readCookie("page-footer-copyright") == "true") {
            $('#checkbox_copyright').prop('checked', true);
            $('.page-footer').css("display","block");
        }
        //如果没有cookie则从数据库的设置中读取，待实现。
    }
    /* hd js end*/
    ...
    /* hd js begin - add by gcm*/
    /* Copyright选中/取消时，显示/隐藏页脚*/
    $('#checkbox_copyright')
        .change(function () {
            if (($('#checkbox_copyright').is(":checked"))){
                $('.page-footer').css("display","block");
            }else{
                $('.page-footer')
                    .css("display","none");
...
function setCookiesForFixedSettings() {
    ...
    /* hd js begin - add by gcm*/
    /* 增加Copyright cookie*/
    createCookie("page-footer-copyright", $('#checkbox_copyright').is(':checked'), 100);
    /* hd js end*/
...
//验证必填项
function required_validate(failWords){
  var num=0;
  $(".input-required").each(function(n){
    if($(this).val() == undefined || $(this).val().trim() == "")
    {
      $(this).toggleClass('input-required-failed')
      num++;
    }
  });
  if(num>0)
  {
    Notify(failWords, 'top-right', '5000', 'danger', 'fa-bolt', true);
    return false;
  }
  else
  {
