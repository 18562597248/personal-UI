<template>
    <div :class="classes" v-clickoutside="handleClose">
        <div
            ...
            ref="reference"
            @click="toggleMenu">
            <div class="hd-tag" v-for="(item, index) in selectedMultiple">
                <span class="hd-tag-text">{{ item.label }}</span>
                <hd-icon type="fa fa-close" @click.native.stop="removeTag(index)"></hd-icon>
            </div>
            <span :class="[prefixCls + '-placeholder']" v-show="showPlaceholder && !filterable">请选择</span>
            ...
            <input
                ...
                @keydown.left.prevent="quit"
                ref="input">
              <div :class="[prefixCls + '-arrow']">
                <hd-icon type="fa fa-close"  v-show="showCloseIcon" @click.native.stop="clearSingleSelect"></hd-icon>
             </div>
             <div :class="[prefixCls + '-arrow']">
                <hd-icon type="fa fa-chevron-down"></hd-icon>
             </div>
        </div>
        <transition :name="transitionName">
            <Drop v-show="dropVisible" :placement="placement" ref="dropdown">
                <ul v-show="notFountShow" :class="[prefixCls + '-not-found']"><li class="hd-multisearch-leftitem-li">无匹配的数据</li></ul>
                <ul v-show="(!notFound && !remote) || (remote && !loading && !notFound)" :class="[prefixCls + '-dropdown-list']"><slot></slot></ul>
                <ul v-show="loading" :class="[prefixCls + '-loading']">加载中</ul>
            </Drop>
        </transition>
    </div>
</template>
<script>
    import Drop from 'components/date/other/DropDown.vue';
    import hdIcon from 'components/icon'
    import clickoutside from 'src/utils/clickoutside';
    import { oneOf, findComponentDownward } from 'src/utils/assist';
    import Emitter from 'src/utils/emitter';
    import {t} from 'src/utils/locale';
    import {debounce} from './utils';

    const prefixCls = 'hd-select';

    export default {
        name: 'iSelect',
      componentName: 'iSelect',
        mixins: [ Emitter ],
        components: {  Drop,hdIcon },
        ...
        computed: {
            ...
            localePlaceholder () {
                if (this.placeholder === undefined) {
                    return t('hd.select.placeholder');
                } else {
        ...
        methods: {
            ...
            toggleMultipleSelected (value, init = false) {
                if (this.multiple) {
                    ...
                    if (!init) {
                        if (this.labelInValue) {
                            this.$emit('change', this.currentValue,hybridValue);
