        ...
        <div ref="reference" :class="[prefixCls + '-rel']">
            <slot>
                <PickerInput
                    ...
                    :icon="iconType"></PickerInput>
            </slot>
        </div>
        <transition :name="transition">
            <DropDown v-show="opened" :placement="placement" ref="drop">
                <div ref="picker"></div>
            </DropDown>
        </transition>
    </div>
...
<script>
    ...
    const prefixCls = 'hd-date-picker';
    ...
    export default {
        name: 'CalendarPicker',
        mixins: [ Emitter ],
        components: { PickerInput, DropDown },
...
//                if (!this.disableClickOutSide) this.visible = false;
            ...
            handleInputChange (event) {
                ...
                if (type === 'daterange' || type === 'timerange' || type === 'datetimerange' || type === 'week') {
                   ...
                   if(value.length<8){
                    ...
                    correctDate = parseDate(value, format);
                }
            ...
            emitChange (date) {
                ...
                this.$nextTick(() => {
                    this.dispatch('hd-formitem', 'form.change', newDate);
                });
            },
            formattingDate (date) {
                const type = this.type;
